...
export {
  ...
  "DualVariable",
  --Types and keys
  "ConormalRing","Factors","Coordinates",
  --More Types
  ...
  -- To delete
  "DualCoordinates", "PrimalCoordinates", "PrimalRing", "DualRing"
}
...
LagrangeRing = new Type of ConormalRing;---Change this to a ring. 
...
lagrangeRing = method(Options => options conormalRing ++ {LagrangeVariable => null})
lagrangeRing(ZZ,Ring) := LagrangeRing => opts -> (c,R) -> (
  ...
  cnRing := conormalRing(R, DualVariable => opts.DualVariable);
  new LagrangeRing from {
    Ring => cnRing#Ring ** lagrangeR,
    Factors => cnRing.Factors | {lagrangeR},
    Coordinates => cnRing.Coordinates | {gens lagrangeR}
  }
)
