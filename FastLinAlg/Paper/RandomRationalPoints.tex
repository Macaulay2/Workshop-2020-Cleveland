...
\section{Our primary purpose: {\tt randomPoints}}\label{randomPoints}
    ...
    \item {\tt LinearIntersection} intersects with a random linear space.  Within this strategy, one can also specify the option {\tt DecompositionStrategy}.  Valid values are {\tt Decompose} and {\tt MultiplicationTable}, the latter of which is currently only implemented for homogeneous ideals.  
    
    The point is, after intersecting the linear space and obtaining an ideal defining a set of (possibly thickened) points, we need to find the minimal associated primes.  By default we use Macaulay2's built-in {\tt decompose} command.  We also have implemented a {\tt MultiplicationTable} algorithm, as provided by Frank Olaf-Schreyer, which utilizes the fact that we can compute the action of a variable on the residue fields of these points in more than one way.  This method is frequently faster for smaller numbers of variables.
    \end{itemize}
...
i6 : randomPoints(J,Strategy=>LinearIntersection)
    -- used 0.0146932 seconds
...
\begin{remark}[Projecting to a hypersurface first]
    ...
    However, in our experience, doing this generic projection first yields slower results. First, one has to compute the dimension.  Furthermore, there are numerous cases where computing this hypersurface $h(X)$ can be extremely slow.  This particularly appears in cases when one is computing successive minors to identify the locus where some variety is nonsingular.  Thus we no longer have an option to project to a hypersurface first.  

    On the other hand, instead of using a truly random linear space to intersect with, in the default strategy, we normally choose a linear space whose defining equations are particularly non-complicated (i.e., with many linear or binomial terms).  Such simple linear spaces are the ones implicitly considered in {\tt randomKRationalPoint} for instance.  In practice, our approach seems to give at superior performance compared to projecting to a hypersurface, without the chance of the code getting hung up on the generic projection or dimension computations.
\end{remark}


\section{Two related functions: {\tt dimViaBezout} and {\tt randomCoordinateChange}}{\label{helper}}

\subsection*{\tt dimViaBezout:} We thank the reviewer for pointing out that in most of the computations, computing the codimension of the given ideal, significantly slows down some computations. While we have avoided most dimension computations in our current implementation, we have also implemented a probibalistic dimension computation of $V(I)$.
...
\begin{itemize}
	...
	If the ambient field is smaller than this integer value, it will automatically be replaced with an extension field. The user may set the {\tt MinimumFieldSize} to ensure that the field being worked over is big enough.  For instance, there are relatively few linear spaces over a field of characteristic $2$, and this can cause incorrect results to be returned to the user. 
    \item {\tt Homogeneous}: 
    If the ideal is homogeneous, we can use homogeneous linear spaces to compute dimension, which has speed advantages.
